...
public class MaxCamera : MonoBehaviour
{
    ...
    [HideInInspector]
    public bool slowAnimWhenMouseInput = false;
    [HideInInspector]
    public float accspeed = 0.001f;//speed
    [Range(0.5f, 3f)]
    public float aspeed = 1.5f;//speed
...
//#if ENABLE_LEGACY_INPUT_MANAGER
        ...
        if (anim)
        {
            bool inp = Vector3.Magnitude(Input.mouseScrollDelta) > 0;//Input.AnyKey;
            rotspeed = Mathf.Clamp(rotspeed + (inp&&slowAnimWhenMouseInput? -accspeed: accspeed), 0, 0.2f);
            //if(!Input.anyKey)
            xDeg += aspeed*rotspeed;
            desiredRotation = Quaternion.Euler(yDeg, xDeg, 0);
        }
